...
export const CATALOG = [
  ...
  // Prix FIXE et non à la minute : le règlement x402 précède la réponse, donc
  // l'agent ne peut pas connaître la durée avant de payer. À 0,02 $ l'appel et
  // 0,00067 $ la minute chez Groq, même une conférence d'une heure reste
  // bénéficiaire.
  { route: "GET /v1/transcribe", price: "$0.02", desc: "Transcribe the actual audio of a YouTube, TikTok or Instagram video with Whisper large-v3. Not caption scraping: the audio is downloaded and run through speech recognition, so it works on videos with no subtitles, in any language, and on TikTok and Instagram where no caption track exists at all. Extraction runs from a real residential IP, reaching sources that refuse datacenter ranges. Returns full text plus sentence-level timestamps.",
    bazaar: { method: "GET", input: { url: "https://www.tiktok.com/@nasa/video/7670721000471891214" }, output: { example: { platform: "tiktok", language: "en", duration_seconds: 29.9, transcript: "Booster ignition and lift off, the crew of Artemis II now bound for the moon.", source: "audio-asr" } } } },
  ...
  { route: "GET /v1/proxy/port/1d", price: "$10.00", desc: "Rent a dedicated mobile port for 24 hours on a real 4G/5G carrier IP (Orange, France and Guadeloupe, AS16028), up to 5 GB included. The entry ticket: qualify the exit against your own target before committing to a week or a month. You rent the line, not a slice of a shared pool — one physical SIM, one client, one address for the full day. Carrier, ASN and uptime are probed every 10 minutes and returned with the key; if no mobile exit is verified live, you are not charged.",
    bazaar: { method: "GET", input: {}, output: { example: { key: "rp1.5.1790000000.xxxx", gb: 5, valid_days: 1, tier: "mobile" } } } },
